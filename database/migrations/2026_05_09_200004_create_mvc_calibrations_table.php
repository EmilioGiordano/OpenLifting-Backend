...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create("mvc_calibrations", function (Blueprint $table) {
            $table->id();
            $table
                ->foreignId("athlete_profile_id")
                ->nullable()
                ->constrained("athlete_profiles")
                ->restrictOnDelete();
            $table
                ->foreignId("guest_profile_id")
                ->nullable()
        ...
        foreach ($this->muscles as $muscle) {
            foreach ($this->sides as $side) {
                $col = "{$muscle}_{$side}";
                DB::statement(
                    "ALTER TABLE mvc_calibrations ADD CONSTRAINT chk_mvc_{$col} CHECK ({$col} > 0 AND {$col} <= 100)",
                );
            }
        }
        ...
        DB::statement(
            "CREATE UNIQUE INDEX uq_mvc_calibrations_athlete ON mvc_calibrations (athlete_profile_id) WHERE athlete_profile_id IS NOT NULL",
        );
        DB::statement(
            "CREATE UNIQUE INDEX uq_mvc_calibrations_guest   ON mvc_calibrations (guest_profile_id)   WHERE guest_profile_id   IS NOT NULL",
        );
    }
